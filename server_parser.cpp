...
bool Parser::number(int &number) {
	...
	if 	(strNumber != "") {
		//std::cout << "STOF: " << strNumber << std::endl;
		number = std::stoi(strNumber);//std::stof(strNumber);
		//std::cout << "RESULTADO CONVERSION: " << number << std::endl;
		return true;
	} else
		return false;
}
