...
'Mismo ejemplo que la prueba 13 pero se sobrecargan metodos nativos * + y print.' print. '\n' print. '\n' print.

lobby _AddSlots:
(|
	punto =
	(|
		print = ( || '(x = ' print. x print. '; y = ' print. y print. ')' print.).
		* = (| :ptr. temp. | temp: (self clone). temp x: (x * ptr). temp y: (y * ptr). temp.).
		setX = (| :valor. | self x: valor.).
		setY = (| :valor. | self y: valor.).
		+ = (| :otroPunto. temp. | temp: (self clone). temp x: (x + (otroPunto x)). temp y: (y + (otroPunto y)). temp.).
	|).
|).
